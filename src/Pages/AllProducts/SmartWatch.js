...
const SmartWatch = () => {
    const { smartWatches, smartWatchesLoading } = useContext(DataContext)

    if (smartWatchesLoading) {
        return <Loader />
    }
    return (
        <div className='grid gap-12 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 px-4 pb-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-12 lg:px-8 lg:py-12'>
            {
                smartWatches.map(smartWatch => <Products allProduct={smartWatch} key={smartWatch._id} />)
            }
        </div>
    );
};
